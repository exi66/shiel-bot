  ...
  async chatInputRun(interaction) {
    ...
    if (!user)
      return await interaction.reply({
        content:
          "Ваш профиль не создан. Для создания используйте комманду `/settings`",
      });
    ...
    if (subcommand === "coupones") {
      ...
      await interaction.reply({
        content: `Уведомления о купонах теперь ${
          user.notifications.coupones ? "включены" : "выключены"
        }!`,
      });
      ...
      global.saveUser(interaction.user.id);
      await interaction.reply({
        content: `Уведомления аукциона теперь ${
          user.notifications.queue ? "включены" : "выключены"
        }!`,
      });
    }
  }
